...
```bash
lambda-playground$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-playground$ sam local start-api
lambda-playground$ curl http://localhost:3000/
```
...
```bash
lambda-playground$ sam logs -n HelloWorldFunction --stack-name lambda-playground --tail
```
...
```bash
lambda-playground$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name lambda-playground
```
